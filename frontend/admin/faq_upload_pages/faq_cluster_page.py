...
def on_change():
    ...
    with st.spinner("Please wait..."):
        response = session.get(
            f'https://l7f99zws-8000.use.devtunnels.ms/api/faqs/total-number-unclustered-queries?category={category}',
...
def on_submit(category):
    ...
    x_api_key = os.getenv("ADMIN_API_KEY")
    response = session.get(
        f'https://l7f99zws-8000.use.devtunnels.ms/api/faqs/kmeans-cluster?category={category}',
        headers={"x-api-key": x_api_key}, cookies={"access_token": token}
    )
    if response.status_code == 200:
        data = response.json()
