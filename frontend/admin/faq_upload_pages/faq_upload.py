...
def on_submit(faq_docs):
    ...
    if st.session_state.clustered_query == False:
        response = session.post("http://127.0.0.1:8000/api/faqs/create-faq", json={"faq_docs": faq_docs}, headers={"x-api-key": x_api_key}, cookies={"access_token": token})
    else:
        response = session.post("http://127.0.0.1:8000/api/faqs/create-faq", json={"faq_docs": faq_docs, "mongo_db_faq_ids": st.session_state.faq_kmeans_docs[0]["ids"]}, headers={"x-api-key": x_api_key}, cookies={"access_token": token})
