...
def on_submit(faq_docs):
    ...
    if st.session_state.clustered_query == False:
        response = session.post("https://l7f99zws-8000.use.devtunnels.ms/api/faqs/create-faq", json={"faq_docs": faq_docs}, headers={"x-api-key": x_api_key}, cookies={"access_token": token})
    else:
        response = session.post("https://l7f99zws-8000.use.devtunnels.ms/api/faqs/create-faq", json={"faq_docs": faq_docs, "mongo_db_faq_ids": st.session_state.faq_kmeans_docs[0]["ids"]}, headers={"x-api-key": x_api_key}, cookies={"access_token": token})
