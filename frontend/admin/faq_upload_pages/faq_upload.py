...
def get_user_inputs():
    ...
    if 'clustered_query' in st.session_state and st.session_state.clustered_query:
        st.session_state.num_questions = len(
            st.session_state.faq_kmeans_docs[0]["questions"])
    for i in range(st.session_state.num_questions):
        ...
        with st.expander(f"Question {i + 1}", expanded=True):
            ...
            st.session_state[faq_doc_key]["categories"] = st.text_input(
                "Categories *:", st.session_state[faq_doc_key]["categories"], key=f"categories_{i}")
            st.session_state[faq_doc_key]["faq_id"] = st.text_input(
                "Faq ID *:", st.session_state[faq_doc_key]["faq_id"], key=f"faq_id_{i}")
            st.session_state[faq_doc_key]["question"] = st.text_area(
                "Question *:", st.session_state[faq_doc_key]["question"], key=f"question_{i}")
            st.session_state[faq_doc_key]["answer"] = st.text_area(
                "Answer *:", st.session_state[faq_doc_key]["answer"], key=f"answer_{i}")
            ...
            c1.button("Add", on_click=partial(
                handle_add_hyperlink_click, faq_doc_key), key=f"add_hyperlink_{i}")
            if st.session_state[faq_doc_key]["num_hyperlinks"] > 1:
                c2.button("Remove", on_click=partial(
                    handle_remove_hyperlink_click, faq_doc_key), key=f"remove_hyperlink_{i}")
            ...
            for j in range(st.session_state[faq_doc_key]["num_hyperlinks"]):
                with hyperlink_col:
                    edited_hyperlink = st.text_area(
                        f"Hyperlink:", key=f"hyperlink_q{i}_{j}", label_visibility="collapsed", placeholder="https://www.example.com")
                with text_col:
                    edited_hyperlink_text = st.text_area(
                        f"Hyperlink Text:", key=f"hyperlink_text_q{i}_{j}", label_visibility="collapsed", placeholder="Example")
                ...
                if edited_hyperlink != "" and edited_hyperlink_text != "":
                    ...
                    st.session_state[faq_doc_key]["hyperlinks"].append(
                        combined_hyperlink)
    ...
    if st.session_state.clustered_query == False:
        ...
        if st.session_state.num_questions > 1:
            col2.button("Remove question",
                        on_click=handle_remove_question_click)
...
def on_submit(faq_docs):
    ...
    if st.session_state.clustered_query == False:
        response = session.post("https://canada-immigration-consultant.onrender.com/api/faqs/create-faq", json={
                                "faq_docs": faq_docs}, headers={"x-api-key": x_api_key}, cookies={"access_token": token})
    else:
        response = session.post("https://canada-immigration-consultant.onrender.com/api/faqs/create-faq", json={
                                "faq_docs": faq_docs, "mongo_db_faq_ids": st.session_state.faq_kmeans_docs[0]["ids"]}, headers={"x-api-key": x_api_key}, cookies={"access_token": token})
...
def on_remove_doc(doc_key, index_in_faq_kmeans_docs):
    ...
    if 'faq_kmeans_docs' in st.session_state and len(st.session_state.faq_kmeans_docs) > 0:
        if index_in_faq_kmeans_docs < len(st.session_state.faq_kmeans_docs[0]["questions"]):
            st.session_state.faq_kmeans_docs[0]["questions"].pop(
                index_in_faq_kmeans_docs)
