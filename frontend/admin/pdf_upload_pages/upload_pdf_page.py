...
def upload_pdf_page():
    ...
    st.button("Upload", on_click=lambda: on_submit(option, update_pdf_id,
              category, skip_tags, inline_txt_removed, uploaded_file, x_api_key))
...
def get_user_inputs():
    ...
    option = st.selectbox(
        "Select an option *",
        ("-- Please select one --", "Add a new PDF document",
        ...
        with st.spinner("PDF is being processed..."):
            try:
                ...
                response = session.post(
                    "https://canada-immigration-consultant.onrender.com/api/upload-pdf", headers=headers, files=file, data=data)
...
def handle_response():
    if st.session_state.processing_done and st.session_state.error == False:
        ...
        error_message = st.error(
            "⚠ There was an error processing the PDF. Please try again.")
        time.sleep(5)
        error_message.empty()
