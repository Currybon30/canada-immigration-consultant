...
def get_user_inputs():
    ...
    st.button("Submit", on_click=lambda: change_password(
...
def change_password(current_password, new_password, confirm_new_password):
    ...
    response = session.put("https://canada-immigration-consultant.onrender.com/api/users/update-password", json={
                           "new_password": new_password, "current_password": current_password}, headers={"Authorization": f"Bearer {token}"})
